...
def ensure_owner(cp: Counterparty, buyer_id: int):
    if cp.buyer_id != buyer_id:
...
async def list_my_counterparties(
    ...
    q = select(Counterparty).filter(Counterparty.buyer_id == current_buyer.id).order_by(Counterparty.short_name)
...
async def create_counterparty(
    ...
    q = select(Counterparty).filter(
        Counterparty.buyer_id==current_buyer.id,
    ...
    cp = Counterparty(buyer_id=current_buyer.id, **payload.model_dump())
    db.add(cp)
    await db.commit()
    await db.refresh(cp)
    return cp
...
async def get_counterparty_by_inn(
    ...
    q = select(Counterparty).filter(
        Counterparty.buyer_id == current_buyer.id, Counterparty.inn == inn
    )
